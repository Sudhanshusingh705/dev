...
const MassCommunicationColleges = () => {
      ...
      try {
        ...
        setTimeout(() => {
          try {
            ...
            const data = generateCollegeData('Mass Communication');
            collegeDataRef.current = data;
            setCollegeData(data);
            setLoading(false);
            dataFetchedRef.current = true;
          } catch (error) {
            console.error("Error loading Mass Communication colleges:", error);
            setError("Failed to load college data. Please try again.");
            setLoading(false);
          }
        }, 0);
      } catch (error) {
        console.error("Error setting up Mass Communication colleges:", error);
        setError("Failed to load college data. Please try again.");
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <View style={styles.centerContainer}>
        ...
        <Text style={styles.loadingText}>Loading Mass Communication colleges...</Text>
      </View>
    );
  }
...
export default MassCommunicationColleges;
