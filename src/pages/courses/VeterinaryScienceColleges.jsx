...
const VeterinaryScienceColleges = () => {
      ...
      try {
        ...
        setTimeout(() => {
          try {
            ...
            const data = generateCollegeData('Veterinary Science');
            collegeDataRef.current = data;
            setCollegeData(data);
            setLoading(false);
            dataFetchedRef.current = true;
          } catch (error) {
            console.error("Error loading Veterinary Science colleges:", error);
            setError("Failed to load college data. Please try again.");
            setLoading(false);
          }
        }, 0);
      } catch (error) {
        console.error("Error setting up Veterinary Science colleges:", error);
        setError("Failed to load college data. Please try again.");
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <View style={styles.centerContainer}>
        ...
        <Text style={styles.loadingText}>Loading Veterinary Science colleges...</Text>
      </View>
    );
  }
...
export default VeterinaryScienceColleges;
