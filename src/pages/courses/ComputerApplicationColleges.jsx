...
const ComputerApplicationColleges = () => {
      ...
      try {
        ...
        setTimeout(() => {
          try {
            ...
            const data = generateCollegeData('Computer Application');
            collegeDataRef.current = data;
            setCollegeData(data);
            setLoading(false);
            dataFetchedRef.current = true;
          } catch (error) {
            console.error("Error loading Computer Application colleges:", error);
            setError("Failed to load college data. Please try again.");
            setLoading(false);
          }
        }, 0);
      } catch (error) {
        console.error("Error setting up Computer Application colleges:", error);
        setError("Failed to load college data. Please try again.");
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <View style={styles.centerContainer}>
        ...
        <Text style={styles.loadingText}>Loading Computer Application colleges...</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      <CollegeList 
        ...
        streamType="Computer Application"
      />
    </View>
  );
};
...
export default ComputerApplicationColleges;
