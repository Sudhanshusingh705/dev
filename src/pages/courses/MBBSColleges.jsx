      ...
      try {
        ...
        setTimeout(() => {
          try {
            ...
            const data = generateCollegeData('MBBS');
            collegeDataRef.current = data;
            setCollegeData(data);
            setLoading(false);
            dataFetchedRef.current = true;
          } catch (error) {
            console.error("Error loading MBBS colleges:", error);
            setError("Failed to load college data. Please try again.");
            setLoading(false);
          }
        }, 0);
      } catch (error) {
        console.error("Error setting up MBBS colleges:", error);
        setError("Failed to load college data. Please try again.");
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <View style={styles.centerContainer}>
        ...
        <Text style={styles.loadingText}>Loading MBBS colleges...</Text>
      </View>
    );
  }
